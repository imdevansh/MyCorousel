...
open class CardSliderViewController: UIViewController, UIScrollViewDelegate {
    ...
    private var backButton: UIButton {
        let backBtn = UIButton(frame: CGRect(x:self.view.frame.size.width-100, y:65, width: 100, height: 50))
        backBtn.tag = 100;
    ...
    private var sendButton: UIButton {
        let sendBtn = UIButton(frame: CGRect(x:self.view.frame.size.width-100, y:self.view.frame.size.height-100, width: 100, height:50))
        sendBtn.addTarget(self, action: #selector(didTapSendButton), for: .touchUpInside)
        sendBtn.setTitle("Send Mail", for: .normal)
...
//        self.dismiss(animated: true, completion: nil)
        let mailHooks = "instagram"
        let mailUrl = URL(string: mailHooks)!
        if UIApplication.shared.canOpenURL(mailUrl)
        {
            UIApplication.shared.open(mailUrl)
        } else {
            //redirect to safari because the user doesn't have gmail app 
            UIApplication.shared.open(URL(string: "http://gmail.com/")!)
        }
    }
